...
def data_classification_report(dl, model, step, device, neighbors):
    ...
    oracle = ExampleOracle(ytrain, max_queries_cnt=10)
    active_learner = MinMax(n_clusters=62)
    active_learner.fit(xtrain, oracle=oracle)
    pairwise_constraints = active_learner.pairwise_constraints_

    clusterer = MPCKMeansMF(n_clusters=62)
    clusterer.fit(xtrain, ml=pairwise_constraints[0], cl=pairwise_constraints[1])

    ypred = clusterer.predict(xtest)
