...
########## SAVE FOLDER ##########
config.save_path = save_folder = 'results/deep_metric_learning/{datasetname}/{modelname}_{datasetname}_{loss}_es{es}_ts{ts}_bs{bs}_s{seed}'.format(datasetname=config.dataset,
...
for epoch in range(config.n_epochs):
    ...
    if (np.mean(current_batch_loss) < minimun_loss):
        ...
        model.eval()
        utils.dml_classification_report.dml_data_classification_report(model, dl_test, device, 7, epoch, config.save_path)
        model.train()
