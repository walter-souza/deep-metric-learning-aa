...
def dml_view_data(dl, model, path_file, name_file, device):
    model.eval()
    xall = None
    yall = []
    with torch.no_grad():
...
def traditional_view_data(dl, model, path_file, name_file, device):
    model.eval()
    xall = None
    yall = []
    with torch.no_grad():
        for ii,item in enumerate(dl):
            ...
            if xall is None:
                xall = embeddings.logits.data.cpu().numpy()
            else:
